...
Vertices primitives::plane(int rows, int columns, float width, float height, const glm::vec4 &color) {
...
Vertices primitives::surface(int p, int q, SurfaceFunction &&f, const glm::vec4 &color) {
    Vertices vertices;
    ...
    for (int j = 0; j <= q; j++) {
        for (int i = 0; i <= p; i++) {
            ...
            Vertex vertex{};
            vertex.position = glm::vec4(position, 1.0);
